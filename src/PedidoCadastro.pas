...
type
  ...
  private
    ...
    function ComboXSaladaAdicionado(): Boolean;
  public
    { Public declarations }
...
begin
  Mensagem := '';
  ValorDesconto := 0;
  FValor := StrToFloatDef(EditValor.Text, 0);
  ...
  begin
    ...
    if not CheckBoxIngredienteAdicional.Enabled then
      begin
        CheckBoxIngredienteAdicional.Enabled := True;
        CheckBoxIngredienteAdicional.Checked := True;
      end
    else
      if not CheckBoxIngredienteAdicional.Checked then
         CheckBoxIngredienteAdicional.Checked := True;

    if Dados.ConsultaIngrediente.Locate('NomeIngrediente', 'REFRIGERANTE', [loPartialKey]) then
    begin
      ComboBoxIngrediente.KeyValue := Dados.ConsultaIngrediente.FieldByName('IdIngrediente').AsInteger;
...
function TTelaPedidoCadastro.ComboXSaladaAdicionado(): Boolean;
var
  Achei: Boolean;
begin
  Achei := False;
  ...
  while not LanchesAdicionados.Eof and not Achei do
  begin
    Achei := Pos('XSALADA', LanchesAdicionados.FieldByName('NomeLanche').AsString) > 0;
    LanchesAdicionados.Next;
  end;
