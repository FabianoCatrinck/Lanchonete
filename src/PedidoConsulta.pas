...
begin
  ...
  try
    try
      ...
      Dados.ApagarLanchesAdicionados(IdPedido);
...
begin
  ...
  try
    ...
    TelaPedidoCadastro.IdPedido := Dados.ConsultaPedido.FieldByName('IdPedido').AsInteger;
    TelaPedidoCadastro.IdCliente := Dados.ConsultaPedido.FieldByName('IdCliente').AsInteger;
    TelaPedidoCadastro.Valor := Dados.ConsultaPedido.FieldByName('Valor').AsFloat;
    TelaPedidoCadastro.Data := Dados.ConsultaPedido.FieldByName('Data').AsDateTime;
    TelaPedidoCadastro.IngredienteAdicional := Dados.ConsultaPedido.FieldByName('IngredienteAdicional').AsString;
...
procedure TTelaPedidoConsulta.BotaoExcluirClick(Sender: TObject);
begin
  if not Dados.ConsultaPedido.IsEmpty then
    if MessageDlg('Confirma a exclusão do pedido?', mtConfirmation, [mbYes,mbNo], 0) = mrYes then
    begin
      ApagarPedido(Dados.ConsultaPedido.FieldByName('IdPedido').AsInteger);
      Dados.ConsultaPedido.Close;
      Dados.ConsultaPedido.Open;
    end;
end;
